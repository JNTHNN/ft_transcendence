...
export default async function View() {
  ...
  const renderForm = () => {
    form.innerHTML = `
      <h1 class="font-display font-black text-2xl md:text-3xl lg:text-4xl font-bold text-text mb-4 md:mb-6">${t('auth.login')}</h1>
      <div class="mb-4">
      ...
      <button type="submit" id="loginBtn" class="w-full bg-sec hover:bg-opacity-80 text-text font-sans font-bold py-2 px-4 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base">
        ${t('auth.login')}
      </button>
