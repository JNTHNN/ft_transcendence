let action = document.getElementById("play") as HTMLElement;
if (!action)
    throw new Error("Element #play introuvable");
let	base   = document.getElementById("game") as HTMLElement;
if (!base)
...
function    showForm() {
    ...
    if (backFormBtn) {
        backFormBtn.addEventListener('click', () => {
            ...
            if (signupDiv) signupDiv.addEventListener('click', () => { showForm(); });
        });
    }
